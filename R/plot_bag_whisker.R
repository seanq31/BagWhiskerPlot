...
.bp_poly_df <- function(poly, group) {
  if (is.null(poly) || length(poly) == 0 || is.vector(poly)) {
    return(NULL)
  }
  data.frame(
    x = poly[, 1],
...
.bp_fade_segments_df <- function(x, y, xend, yend,
  ...
  if (length(x) == 0) {
    return(data.frame(x = numeric(0), y = numeric(0), xend = numeric(0), yend = numeric(0), alpha = numeric(0)))
  }
...
.bp_points_outside_poly <- function(pts, poly, tol = 1e-8) {
  ...
  point_on_segment <- function(px, py, x1, y1, x2, y2, tol) {
    ...
    cross <- vx * wy - vy * wx
    if (abs(cross) > tol * (abs(vx) + abs(vy) + 1)) {
      return(FALSE)
    }
...
bp_build_layers <- function(
  ...
  if (isTRUE(is.one.dim)) {
    ...
    if (ROT[1, 1] == 0) {
      ...
      rng <- c(diff(usrX), diff(usrY))
      s <- 0.15 * min(rng) / sqrt(sum(vec.ortho^2))
    ...
    layers <- c(layers, list(
      ggplot2::geom_polygon(
        data = box.df, ggplot2::aes(x = x, y = y, group = group),
    ...
    layers <- c(layers, list(
      ggplot2::geom_point(
        data = data.frame(x = five[c(1, 5), 1], y = five[c(1, 5), 2]),
        ggplot2::aes(x = x, y = y),
    ...
    if (!is.null(out.df) && nrow(out.df)) {
      layers <- c(layers, list(
        ggplot2::geom_point(
          data = out.df, ggplot2::aes(x = x, y = y),
  ...
  if (show.loophull && !is.null(hull.loop) && length(hull.loop) > 0) {
    ...
    layers <- c(layers, list(
      ggplot2::geom_polygon(
        data = df.loop, ggplot2::aes(x = x, y = y, group = group),
  ...
  if (show.looppoints && !is.null(pxy.outer) && length(pxy.outer) > 0) {
    ...
    if (any(!outside_mask)) {
      layers <- c(layers, list(
        ggplot2::geom_point(
          data = df.lpts[!outside_mask, , drop = FALSE], ggplot2::aes(x = x, y = y),
  ...
  if (show.baghull && !is.null(hull.bag) && length(hull.bag) > 0) {
    ...
    layers <- c(layers, list(
      ggplot2::geom_polygon(
        data = df.bag, ggplot2::aes(x = x, y = y, group = group),
        fill = col.baghull, color = "black", linewidth = 0.5
      )
    ))
  }
  ...
  if (show.fence_mag_bag && !is.null(fence_mag_bag) && length(fence_mag_bag) > 0) {
    ...
    layers <- c(layers, list(
      ggplot2::geom_polygon(
        data = df.fence, ggplot2::aes(x = x, y = y, group = group),
  ...
  if (show.bagpoints && !is.null(pxy.bag) && length(pxy.bag) > 0) {
    ...
    layers <- c(layers, list(
      ggplot2::geom_point(
        data = df.bpts, ggplot2::aes(x = x, y = y),
  ...
  if (show.outlier && !is.null(pxy.outlier) && length(pxy.outlier) > 0) {
    ...
    if (any(!outside_mask_ol)) {
      layers <- c(layers, list(
        ggplot2::geom_point(
          data = df.ol[!outside_mask_ol, , drop = FALSE], ggplot2::aes(x = x, y = y),
