compute.bagWhiskerPlot <- function(
    x, y, type1 = "unadjusted", # the type of type 1 error, other choices are FWER,FDR, PFER
  ...
  find.cut.z.pg <- function(z, pg, center = c(0, 0), debug.plots = "no") {
    ...
    # compute cut points
    cuts <- cut.z.pg(
      z[, 1], z[, 2], pg[segm.no, 1], pg[segm.no, 2],
  ...
  if (nrow(xydata) <= 4) {
    ...
    center <- colMeans(xydata)
    res <- list(
      xy = xy, xydata = xydata, prdata = prdata, hdepths = rep(1, n), hdepth = rep(1, n),
  ...
  if (type1 == "unadjusted") {
    ...
    # browser()
    # par(mfrow = c(1, 2))
    # hist(p_values, ylim=c(0, 800))
    # hist(pchisq(mah_dist_sq, df = 2, lower.tail = FALSE), ylim=c(0, 800))
    ...
    if (normal_outter) {
      ...
      if (sum(testres$de) == 0) {
        ...
        print(mult_factor)
        # print(median(mah_dist_sq))
        # print(max(mah_dist_sq))
        # is_outlier <- mah_dist_sq > mah.cutoff
        hull.loop <- cbind(hull.bag[, 1] - center[1], hull.bag[, 2] - center[2])
  ...
  if(asymp_dist_pv == 'chisq') {
    ...
    stop("Invalid option for asymp_dist_pv. Choose 'chisq' or 'f'.")
  }
  if (type1 %in% c("FWER", "FDR", "PFER")) {
    ...
    lambda_mag_bag <- sqrt(d2_adj / median_d2)
    # browser()
  } else {
    lambda_mag_bag <- factor
    print("Warning: lambda_mag_bag set to factor value in unadjusted method")
  ...
  if (!is.finite(lambda_mag_bag) || lambda_mag_bag == 0) {
    ...
    if (!is.finite(max_ratio) || is.na(max_ratio)) {
      lambda_mag_bag <- factor
      print("Warning: failed to recompute lambda_mag_bag; set to factor.")
    } else {
      lambda_mag_bag <- max_ratio
      print(paste("lambda_mag_bag recomputed to contain all data points:", signif(lambda_mag_bag, 6)))
    }
  }
  # browser()
  # if (!is.finite(lambda_mag_bag)) {
  #   lambda_mag_bag <- factor
  #   print(paste("Warning: lambda_mag_bag is infinite. Set to factor value:", factor))
  # } else if (lambda_mag_bag == 0) {
  #   lambda_mag_bag <- factor
  #   print(paste("Warning: lambda_mag_bag is zero. Set to factor value:", factor))
  # } else {
  #   print(paste("lambda_mag_bag:", lambda_mag_bag))
  # }
  ...
  if (!normal_inlier && type1 %in% c("FWER", "FDR", "PFER")) {
    ...
    if (conservative_lambda == 1) {
      ...
      if (!is.null(hull.bag) && length(hull.bag) > 0 && length(mt_res$de) == nrow(xydata)) {
        idx_in <- !mt_res$de
        if (sum(idx_in) > 0) {
          z <- xydata[as.logical(idx_in), , drop = FALSE]
  ...
  fence_mag_bag <- NULL
  if (!is.null(hull.bag) && length(hull.bag) > 0 && is.finite(lambda_mag_bag)) {
    tmp <- cbind(hull.bag[, 1] - center[1], hull.bag[, 2] - center[2])
    tmp <- lambda_mag_bag * tmp
  ...
  outside_fence_mag_bag <- NULL
  outside_fence_mag_bag_idx <- integer(0)
  if (!is.null(fence_mag_bag) && length(fence_mag_bag) > 0) {
    # Reuse existing convex polygon test helper defined above
    outside_mask <- out.of.polygon(xydata, fence_mag_bag)
    outside_fence_mag_bag <- outside_mask
    outside_fence_mag_bag_idx <- which(isTRUE(outside_mask) | (!is.na(outside_mask) & outside_mask))
  } else {
    outside_fence_mag_bag <- rep(FALSE, nrow(xydata))
    outside_fence_mag_bag_idx <- integer(0)
  }
  ...
  res <- list(
    ...
    xy = xy, xydata = xydata,
    # MT details for reference
    p_values = p_values,
    mah_dist_sq = mah_dist_sq,
    t_adj = mt_res$th,
    d2_adj = d2_adj,
    lambda_mag_bag = lambda_mag_bag,
    mt_method = mt_method,
...
find.hdepths.tp <- function(tp, data, number.of.directions = 181) { # 121130
  ...
  for (j in seq(along = sinphi)) { # print(j)
    ...
    hdtp <- pmin(
      hdtp, (rank(c(tpt, xyt), ties.method = "min"))[1:n.tp]
...
.bp_multi_test_adj_threshold <- function(p_values, method = c("FWER", "FDR", "PFER"), alpha = 0.1) {
  type1 <- match.arg(method)
  if (type1 == "FWER") {
    testres <- holm.func(p_values, alpha)
  } else if (type1 == "FDR") {
    testres <- bh.func(p_values, alpha)
  } else if (type1 == "PFER") {
    testres <- chau.func(p_values, alpha)
  }
  list(th = testres$th, de = testres$de)
}
