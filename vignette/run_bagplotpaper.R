...
for (setting_idx in 1:nrow(all_settings)) {
    ...
    old=Sys.time()
    tmp_plt = bagplot(dat,factor=3,type1='FWER',q=0.1, normal_inlier=normal_inlier,normal_outter=normal_outter,conservative_lambda=conservative_lambda,create.plot=TRUE,approx.limit=10000, redefine_loop=redefine_loop, cex=1, pch=1,
    ...
    old=Sys.time()
    tmp_plt = bagplot(dat,factor=3,type1='FDR',q=0.01,normal_inlier=normal_inlier,normal_outter=normal_outter,conservative_lambda=conservative_lambda,create.plot=TRUE,approx.limit=10000, redefine_loop=redefine_loop, cex=1, pch=1,
            show.outlier=TRUE,show.looppoints=TRUE, whisker.end.prop = 0.7,
            show.bagpoints=TRUE,dkmethod=1,asymp_dist_pv = 'chisq',
            show.whiskers=TRUE,show.loophull=FALSE, center_type = center_type,
            show.baghull=TRUE,verbose=FALSE,col.baghull = "#D3D3D3",
    ...
    old=Sys.time()
    tmp_plt = bagplot(dat,factor=3,type1='PFER',q=0.5, normal_inlier=normal_inlier,normal_outter=normal_outter,conservative_lambda=conservative_lambda,create.plot=TRUE,approx.limit=10000, redefine_loop=redefine_loop, cex=1, pch=1,
            show.outlier=TRUE,show.looppoints=TRUE, whisker.end.prop = 0.7,
            show.bagpoints=TRUE,dkmethod=1,asymp_dist_pv = 'chisq',
            show.whiskers=TRUE,show.loophull=FALSE, center_type = center_type,
            show.baghull=TRUE,verbose=FALSE,col.baghull = "#D3D3D3",
    ...
    ylim_union <- range(unlist(lapply(lims_list, function(l) l$y)), na.rm = TRUE)
        ...
        png(file.path(save_dir, paste0("rd_bagplotpaper/", dist_type), paste0(fig_title, "_aplpack.png")),
