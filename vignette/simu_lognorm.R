...
for (setting_idx in 1:nrow(all_settings)) {
  ...
  old <- Sys.time()
  tmp_plt <- BagWhiskerPlot::bag_whisker(dat,
    type1 = "FDR", q = 0.01, normal_inlier = normal_inlier, normal_outter = normal_outter, conservative_lambda = conservative_lambda, create.plot = TRUE, approx.limit = 10000, redefine_loop = redefine_loop, cex = 0.6, pch = 1,
    show.outlier = TRUE, show.loophull = FALSE,
    show.bagpoints = TRUE, dkmethod = 1, center_type = center_type,
    show.whiskers = TRUE, asymp_dist_pv = "chisq", whisker.end.prop = 0.7,
    show.baghull = TRUE, verbose = FALSE, col.baghull = "#D3D3D3",
  ...
  png(file.path(save_dir, paste0("simu_", dist_type), paste0(fig_title, "_aplpack.png")), width = 3, height = 3, units = "in", res = 300)
